...
def build_market_context(market_context: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "字段说明": {
            ...
            "segment_contexts": "训练窗口按时间顺序切成若干段后的标签摘要，用来保留窗口内部的演变结构，避免只看总均值。",
            ...
            "segment_contexts[].labels": "该分段自己的完整市场标签，可与总 labels 对比，看哪些维度在前中后段发生了演变。",
            ...
            "stability_metrics": "训练窗口内部状态切换的稳定性指标，用来区分稳定环境和来回切换环境。",
            ...
            "temporal_structure_summary_text": "把各分段标签和稳定性指标串起来的时间结构摘要。",
            "selector_similarity": "来自 selector 的关键相似月份摘要，说明当前训练窗口里最像 test 的月份有哪些。",
            "selector_similarity.top_similar_months": "selector 识别出的 Top 相似月份列表。",
            "selector_similarity.core_similar_months_in_train_window": "真正落在当前训练窗口里的核心相似月份。",
            "selector_similarity.summary_text": "关键相似月份的自然语言摘要。",
