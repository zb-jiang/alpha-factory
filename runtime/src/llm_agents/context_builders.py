...
def build_summary_context(llm_summary: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "字段说明": {
            ...
            "fundamental_feature_health": "基本面分析师专用体检，单独评估基本面/风格特征的分位数组合效果，避免基本面特征被全局短期相关性排序淹没。",
            "fundamental_feature_health.features": "每个基本面/风格特征的明细指标列表。",
            "fundamental_feature_health.features[].label_corr": "该特征与收益标签的整体相关系数。",
            "fundamental_feature_health.features[].top_quantile_return": "每天按该特征排序后，特征值最高 20% 股票的平均未来收益标签。",
            "fundamental_feature_health.features[].bottom_quantile_return": "每天按该特征排序后，特征值最低 20% 股票的平均未来收益标签。",
            ...
            "fundamental_feature_health.top_long_short_features": "按 |long_short_return| 排名前列的基本面/风格特征。",
