...
- 作为估值分析专家需要重点关注特征体检报告摘要 【特征体检报告摘要及字段说明】 中 fundamental_feature_health 中的 long_short_return、top_quantile_return、bottom_quantile_return 和 label_corr，判断估值特征在当前收益标签定义下是否有效。而【特征体检报告摘要及字段说明】 中其他与估值无关的摘要数据无需特别关注。
...
- 作为盈利质量分析专家需要重点关注特征体检报告摘要 【特征体检报告摘要及字段说明】 中 fundamental_feature_health 中的 long_short_return、top_quantile_return、bottom_quantile_return 和 label_corr，判断盈利质量特征在当前收益标签定义下是否有效
...
- 作为财务健康分析专家需要重点关注特征体检报告摘要 【特征体检报告摘要及字段说明】 中 fundamental_feature_health 中的 long_short_return、top_quantile_return、bottom_quantile_return 和 label_corr，判断财务健康特征在当前收益标签定义下是否有效
...
- 【当前市场环境及字段说明】描述的是当前样本窗口的市场环境，不是实时行情。
- 阅读【当前市场环境及字段说明】时，应优先结合 segment_contexts、temporal_structure_summary_text、stability_metrics，判断当前样本窗口的前中后阶段发生了什么变化，以及这些变化是持续稳定还是频繁切换。summary_text 和 labels 只用于快速建立整体印象，不能替代上述主证据；其中 rate、macro_liquidity、economy、inflation 这 4 个宏观标签更多是辅助背景，并明确理解为当前样本窗口结束附近的宏观快照，而不是整个样本窗口的平均宏观环境。"""


def _prune_summary_context_for_analyst(agent_id: str, summary_context: dict[str, Any]) -> dict[str, Any]:
    """按分析师角色裁剪特征体检摘要，减少与当前角色无关的噪音。"""
    field_desc = summary_context.get("字段说明")
    data = summary_context.get("数据")
    if not isinstance(field_desc, dict) or not isinstance(data, dict):
        return summary_context

    # 这些字段更适合首席/生成/评审阶段，不必在单个分析师阶段反复干扰。
    field_desc.pop("previous_top", None)
    field_desc.pop("previous_skipped", None)
    data.pop("previous_top", None)
    data.pop("previous_skipped", None)

    if agent_id not in FUNDAMENTAL_ANALYST_IDS:
        for key in [
            "fundamental_feature_health",
            "fundamental_feature_health.features",
            "fundamental_feature_health.features[].label_corr",
            "fundamental_feature_health.features[].top_quantile_return",
            "fundamental_feature_health.features[].bottom_quantile_return",
            "fundamental_feature_health.features[].long_short_return",
            "fundamental_feature_health.features[].valid_observations",
            "fundamental_feature_health.top_long_short_features",
        ]:
            field_desc.pop(key, None)
        data.pop("fundamental_feature_health", None)
...
def _build_analyst_messages(agent_id: str, context: dict[str, Any]) -> list[dict[str, str]]:
    ...
    summary_context = _prune_summary_context_for_analyst(agent_id, summary_context)
